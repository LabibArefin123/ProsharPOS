        ...
        <tbody id="trade-wrapper">
            @foreach ($organization->documents->where('type', 'trade') ?? [] as $document)
                <tr class="trade-row">
                    <td>{{ $tradeIndex + 1 }}</td>
                    <td>
                        <input type="hidden" name="documents[{{ $tradeIndex }}][id]" value="{{ $document->id }}">
                        <input type="hidden" name="documents[{{ $tradeIndex }}][type]" value="trade">
                        <input type="text" name="documents[{{ $tradeIndex }}][number]" class="form-control"
                            value="{{ old('documents.' . $tradeIndex . '.number', $document->number) }}">
                    </td>
                    <td>
                        <input type="date" name="documents[{{ $tradeIndex }}][validity]" class="form-control"
                            value="{{ old('documents.' . $tradeIndex . '.validity', $document->validity) }}">
                    </td>
                    <td>
                        <select name="documents[{{ $tradeIndex }}][financial_year]"
                            id="financialYearSelect_{{ $tradeIndex }}" class="form-control"
                            data-selected="{{ old('documents.' . $tradeIndex . '.financial_year', $document->financial_year ?? '') }}"></select>
                    </td>
                    <td class="text-center">
                        ...
                        @endif
                        <input type="file" name="documents[{{ $tradeIndex }}][document]"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('add-trade').addEventListener('click', function() {
            ...
            <td>
                <select name="documents[${tradeIndex}][financial_year]" id="financialYearSelect_${tradeIndex}" class="form-control"></select>
            </td>
            <td>
                <input type="file" name="documents[${tradeIndex}][document]" class="form-control mt-1">
            </td>
            <td>
                <button type="button" class="btn btn-danger btn-sm remove-trade">Remove</button>
            </td>
