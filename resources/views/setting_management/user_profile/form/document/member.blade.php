        ...
        <tbody id="org-wrapper">
            @foreach ($organization->documents->where('type', 'org_member') ?? [] as $document)
                <tr class="org-row">
                    <td>{{ $orgIndex + 1 }}</td>
                    <td>
                        <input type="hidden" name="documents[{{ $orgIndex }}][id]" value="{{ $document->id }}">
                        <input type="hidden" name="documents[{{ $orgIndex }}][type]"value="org_member">
                        <input type="text" name="documents[{{ $orgIndex }}][number]" class="form-control"
                            value="{{ old('documents.' . $orgIndex . '.number', $document->number) }}">
                    </td>
                    <td>
                        <input type="date" name="documents[{{ $orgIndex }}][validity]" class="form-control"
                            value="{{ old('documents.' . $orgIndex . '.validity', $document->validity) }}">
                    </td>
                    <td>
                        <select name="documents[{{ $orgIndex }}][financial_year]"
                            id="financialYearSelect_{{ $orgIndex }}" class="form-control"
                            data-selected="{{ old('documents.' . $orgIndex . '.financial_year', $document->financial_year ?? '') }}"></select>
                    </td>
                    <td class="text-center">
                        ...
                        @endif
                        <input type="file" name="documents[{{ $orgIndex }}][document]"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('add-org').addEventListener('click', function() {
            ...
            <td>
                <select name="documents[${orgIndex}][financial_year]" id="financialYearSelect_${orgIndex}" class="form-control"></select>
            </td>
            <td>
                <input type="file" name="documents[${orgIndex}][document]" class="form-control mt-1">
            </td>
            <td>
                <button type="button" class="btn btn-danger btn-sm remove-org">Remove</button>
            </td>
