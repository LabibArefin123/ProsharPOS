        ...
        <tbody id="bin-wrapper">
            @foreach ($organization->documents->where('type', 'bin') ?? [] as $document)
                <tr class="bin-row">
                    <td>{{ $binIndex + 1 }}</td>
                    <td>
                        <input type="hidden" name="documents[{{ $binIndex }}][id]" value="{{ $document->id }}">
                        <input type="hidden" name="documents[{{ $binIndex }}][type]" value="bin">
                        <input type="text" name="documents[{{ $binIndex }}][number]" class="form-control"
                            value="{{ old('documents.' . $binIndex . '.number', $document->number) }}">
                    </td>
                    <td>
                        <input type="date" name="documents[{{ $binIndex }}][validity]" class="form-control"
                            value="{{ old('documents.' . $binIndex . '.validity', $document->validity) }}">
                    </td>
                    <td>
                        <select name="documents[{{ $binIndex }}][financial_year]"
                            id="financialYearSelect_{{ $binIndex }}" class="form-control"
                            data-selected="{{ old('documents.' . $binIndex . '.financial_year', $document->financial_year ?? '') }}"></select>
                    </td>
                    <td class="text-center">
                        ...
                        @endif
                        <input type="file" name="documents[{{ $binIndex }}][document]"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('add-bin').addEventListener('click', function() {
            ...
            <td>
                <select name="documents[${binIndex}][financial_year]" id="financialYearSelect_${binIndex}" class="form-control"></select>
            </td>
            <td>
                <input type="file" name="documents[${binIndex}][document]" class="form-control mt-1">
            </td>
            <td>
                <button type="button" class="btn btn-danger btn-sm remove-bin">Remove</button>
            </td>
