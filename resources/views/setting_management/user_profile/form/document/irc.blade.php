        ...
        <tbody id="irc-wrapper">
            @foreach ($organization->documents->where('type', 'irc') ?? [] as $document)
                <tr class="irc-row">
                    <td>{{ $ircIndex + 1 }}</td>
                    <td>
                        <input type="hidden" name="documents[{{ $ircIndex }}][id]" value="{{ $document->id }}">
                        <input type="hidden" name="documents[{{ $ircIndex }}][type]"value="irc">
                        <input type="text" name="documents[{{ $ircIndex }}][number]" class="form-control"
                            value="{{ old('documents.' . $ircIndex . '.number', $document->number) }}">
                    </td>
                    <td>
                        <input type="date" name="documents[{{ $ircIndex }}][validity]" class="form-control"
                            value="{{ old('documents.' . $ircIndex . '.validity', $document->validity) }}">
                    </td>
                    <td>
                        <select name="documents[{{ $ircIndex }}][financial_year]"
                            id="financialYearSelect_{{ $ircIndex }}" class="form-control"
                            data-selected="{{ old('documents.' . $ircIndex . '.financial_year', $document->financial_year ?? '') }}"></select>
                    </td>
                    <td class="text-center">
                        ...
                        @endif
                        <input type="file" name="documents[{{ $ircIndex }}][document]"
        ...
        document.getElementById('add-irc').addEventListener('click', function() {
            ...
            if (rowCount >= maxRows) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Limit Reached',
                    text: 'You are not allowed to give more than 10 documents',
            ...
            <td>
                <select name="documents[${ircIndex}][financial_year]" id="financialYearSelect_${ircIndex}" class="form-control"></select>
            </td>
            <td>
                <input type="file" name="documents[${ircIndex}][document]" class="form-control mt-1">
            </td>
            <td>
                <button type="button" class="btn btn-danger btn-sm remove-org">Remove</button>
            </td>
