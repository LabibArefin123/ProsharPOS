        ...
        <tbody id="tax-wrapper">
            @foreach ($organization?->documents->where('type', 'tax') ?? [] as $document)
                <tr class="tax-row">
                    <td>{{ $taxIndex + 1 }}</td>
                    <td>
                        <input type="hidden" name="documents[{{ $taxIndex }}][id]" value="{{ $document->id }}">
                        <input type="hidden" name="documents[{{ $taxIndex }}][type]" value="tax">
                        <input type="text" name="documents[{{ $taxIndex }}][number]" class="form-control"
                            value="{{ old('documents.' . $taxIndex . '.number', $document->number) }}">
                    </td>
                    <td>
                        <input type="date" name="documents[{{ $taxIndex }}][validity]" class="form-control"
                            value="{{ old('documents.' . $taxIndex . '.validity', $document->validity) }}">
                    </td>
                    <td>
                        <select name="documents[{{ $taxIndex }}][financial_year]"
                            id="financialYearSelect_{{ $taxIndex }}" class="form-control"
                            data-selected="{{ old('documents.' . $taxIndex . '.financial_year', $document->financial_year ?? '') }}"></select>
                    </td>
                    <td class="text-center">
                        ...
                        @endif
                        <input type="file" name="documents[{{ $taxIndex }}][document]"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('add-tax').addEventListener('click', function() {
            ...
            <td>
                <select name="documents[${taxIndex}][financial_year]" id="financialYearSelect_${taxIndex}" class="form-control"></select>
            </td>
            <td>
                <input type="file" name="documents[${taxIndex}][document]" class="form-control mt-1">
            </td>
            <td>
                <button type="button" class="btn btn-danger btn-sm remove-tax">Remove</button>
            </td>
